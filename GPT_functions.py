...
def parse_metabolic_reactions_gpt(text):
    ...
    for reaction_line in reaction_lines:
        ...
        metabolites = combined_reaction.split('+')
        metabolites = [metabolite.strip() for metabolite in metabolites if metabolite.strip()]
        metabolities = ['water' if metabolite.lower() == 'h2o' else metabolite for metabolite in metabolites]
...
def help_format_answer_with_gpt(raw_answer):
    question = "Given the answer: " + raw_answer + ". Only give the chemical reaction(s) and nothing else i.e. no text. Start with '1.' indicating the first reaction "
